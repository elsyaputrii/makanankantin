...
<div class="max-w-7xl mx-auto px-4 py-8">
  ...
  <div class="flex items-center justify-between mb-6">
    ...
    <a href="{{ route('admin.users.create') }}" class="bg-orange-500 hover:bg-orange-600 text-white px-5 py-2 rounded-md shadow text-sm font-semibold">
      + Tambah Staff Baru
    </a>
  </div>
      ...
      <tbody class="bg-cream-100 divide-y divide-orange-100 text-sm text-gray-700">
        ...
        <tr class="hover:bg-cream-200 transition duration-150">
          ...
          <td class="px-6 py-4 flex gap-3">
            ...
            <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus staff ini?')">
              @csrf
              @method('DELETE')
        ...
        @empty
        <tr>
          <td colspan="5" class="px-6 py-6 text-center text-gray-500">Belum ada staff tersedia.</td>
        </tr>
        @endforelse
      </tbody>
    </table>
